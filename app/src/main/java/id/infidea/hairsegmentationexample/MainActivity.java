...
//                });
        processor.addPacketCallback(
                ...
                (packet) -> {
                    ...
                    ArrayList<ArrayList<Integer>> imageRedHorizontal = new ArrayList<>();
                    boolean foundRedCenter = false;
                    int color;
                    for (int j = 0; j <= 511; j++) {
                        color = bitmap.getPixel(j, 255);
...
//                        Log.v(TAG, "Received found true");
                                ...
                                if (found) {
                                    ...
                                    if (mid == low || mid == high) {
                                        ...
                                        Log.v(TAG, "Received found finish"+imageRed);
                                        break;
                                    }
                                } else {
                        ...
                        for (int i = 1; i <= 2; i++) {
                            ...
                            int midHorizontal = (highHorizontal + lowHorizontal) / 2;
                            while (midHorizontal != lowHorizontal && midHorizontal != highHorizontal) {
                                int chosenX = 0;
                                boolean found = false;
                                // x-axis Row check in binary search y-axis
                                for (int j = 0; j <= 511; j++) {
                                    color = bitmap.getPixel(midHorizontal, j);
                                    Float red = Color.valueOf(color).red();
                                    Float alpha = Color.valueOf(color).alpha();
                                    if (red == 1.0f && alpha == 1.0f) {
                                        found = true;
                                ...
                                if (found) {
                                    // If checked row has red color
                                    lowHorizontal = midHorizontal;
                                    midHorizontal = (midHorizontal + highHorizontal) / 2;

                                    if (midHorizontal == lowHorizontal || midHorizontal == highHorizontal) {
                                        ArrayList<Integer> addData = new ArrayList<>();
                                        addData.add(chosenX);
                                        addData.add(midHorizontal);
                                        imageRedHorizontal.add(addData);
                                        Log.v(TAG, "Received found finish hori"+imageRedHorizontal);
                                        break;
                                    }
                                } else {
                                    // If checked row doesnt have red color
                                    highHorizontal = midHorizontal;
                                    midHorizontal = (midHorizontal + lowHorizontal) / 2;
                                }
                            }
                        }
                    }
                    if (imageRed.size() >= 2 && imageRedHorizontal.size() >= 2) {
                        Log.v(TAG, "Received color pos : " + imageRed);
                        Log.v(TAG, "Received color pos hori : " + imageRedHorizontal);
                        float ratioBottom = imageRed.get(0).get(1) / 512.0f;
                        float ratioTop = imageRed.get(1).get(1) / 512.0f;
                        float ratioLeft = imageRedHorizontal.get(0).get(1) / 512.0f;
                        float ratioRight = imageRedHorizontal.get(1).get(1) / 512.0f;
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Log.v(TAG, "Received color top : " + previewDisplayView.getWidth());
                                Log.v(TAG, "Received color bottom : " + previewDisplayView.getHeight());
                                top.setY(ratioTop * previewDisplayView.getHeight());
                                bottom.setY(ratioBottom * previewDisplayView.getHeight());
                                left.setX(ratioLeft * previewDisplayView.getWidth());
                                right.setX(ratioRight * previewDisplayView.getWidth());
                            }
                        });
                    }
